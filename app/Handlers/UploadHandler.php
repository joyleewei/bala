...
class UploadHandler {
    ...
    private function setDriver($driver = null, $config = null){
        // 设置驱动
        $driver = $driver ? : ($this->driver       ? : config('think_upload.file_upload_type'));
        $config = $config ? : ($this->driverConfig ? : config('think_upload.upload_type_config'));
        $class = strpos($driver,'\\')? $driver : 'App\\Handlers\\Upload\\Driver\\'.ucfirst(strtolower($driver));
